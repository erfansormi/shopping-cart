        ...
        <>
            ...
            <div className={`container`}>
                <div className={`${styles.user_info_container} row`}>
                    <div className={`col-xl-8 col-md-7 col-11 mx-auto ms-xl-3 ${styles.left_user_details}`}>

                        {profileState.orders &&
                            <div className={`d-flex align-items-center fw-bold fs-4 justify-content-center`} style={{ height: "220px" }}>
                                orders is empty!
                            </div>
                        }
                        ...
                        {profileState.favorites &&
                            <div className={`row ${styles.favorite_container}`}>
                                {cartState.favoriteList.length ? cartState.favoriteList.map(item =>
                                    <section key={item.id} className={`col-md-9 col-xl-6 col-lg-8 col-11 col-sm-10 mx-auto m-0 px-0 ${styles.favorite_box}`}>
                                        {<FavoriteList key={item.id} id={item.id} item={item}
                                            image={item.image} price={item.price} title={item.title} />}
                                    </section>)
                                    :
                                    <div className={`d-flex align-items-center fw-bold fs-4 justify-content-center`} style={{ height: "220px" }}>
